...
function normalizeVitalStatus(status: string | undefined): 'normal' | 'high' | 'low' | 'critical' {
...
function determineVitalTypeFromPattern(text: string): string {
...
function determineStatusFromVitalValue(type: string, value: string, unit: string): 'normal' | 'high' | 'low' | 'critical' {
  try {
...
function isVitalSignTest(testName: string): boolean {
  const vitalKeywords = [
...
function mapTestToVitalType(testName: string): string {
  if (testName.includes('blood pressure') || testName.includes('bp') || testName.includes('systolic') || testName.includes('diastolic')) {
    return 'blood_pressure';
  }
  if (testName.includes('heart rate') || testName.includes('pulse') || testName.includes('hr') || testName.includes('bpm')) {
    return 'heart_rate';
  }
...
function getVitalIcon(type: string): React.ComponentType<any> {
...
function parseVitalsFromText(text: string, date: string): VitalSign[] {
