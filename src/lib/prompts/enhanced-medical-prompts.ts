...
export const ENHANCED_SYSTEM_PROMPT = `You are an expert medical document analysis AI with extensive knowledge of medical terminology, lab values, prescription formats, and healthcare documentation standards. Your role is to extract comprehensive, structured information from medical documents with high precision.
...
Always respond with a JSON object that matches the specified schema exactly.`;

// Enhanced lab results prompt with comprehensive structure
export const ENHANCED_LAB_RESULTS_PROMPT = `
...
export const ENHANCED_PRESCRIPTION_PROMPT = `
...
// Enhanced radiology prompt
export const ENHANCED_RADIOLOGY_PROMPT = `
...
// Enhanced vitals prompt
export const ENHANCED_VITALS_PROMPT = `
...
export const ENHANCED_GENERAL_MEDICAL_PROMPT = `
...
// Function to get the appropriate enhanced prompt based on report type
export function getEnhancedPromptForReportType(reportType: string): string {
  ...
  switch (normalizedType) {
    ...
    case 'lab results':
    case 'blood work':
      return ENHANCED_LAB_RESULTS_PROMPT;
      
    case 'prescription':
    case 'rx':
    case 'medication':
    case 'pharmacy':
      return ENHANCED_PRESCRIPTION_PROMPT;
    ...
    case 'mri':
    case 'ultrasound':
      return ENHANCED_RADIOLOGY_PROMPT;
      
    case 'vitals':
    case 'vital signs':
    case 'measurements':
      return ENHANCED_VITALS_PROMPT;
      
    default:
      return ENHANCED_GENERAL_MEDICAL_PROMPT;
  }
}

// Enhanced smart tag generation
export function generateEnhancedSmartTags(parsedData: any): string[] {
  const tags: string[] = [];
  ...
  switch (parsedData.reportType) {
    case 'lab':
      if (parsedData.testPanels) {
        parsedData.testPanels.forEach((panel: any) => {
          if (panel.name) tags.push(panel.name);
          if (panel.category) tags.push(panel.category);
        });
      }
      break;
    ...
    case 'prescription':
      if (parsedData.medications) {
        parsedData.medications.forEach((med: any) => {
          if (med.name) tags.push(med.name);
        });
      }
      break;
      
    case 'radiology':
      if (parsedData.modality) tags.push(parsedData.modality);
      if (parsedData.bodyPart) tags.push(parsedData.bodyPart);
      if (parsedData.studyType) tags.push(parsedData.studyType);
      break;
  }
